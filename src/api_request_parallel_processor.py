# ---- 工程集成说明（本仓库维护者补充）----
# 来源: OpenAI Cookbook 的经典限流并行脚本
# （openai-cookbook/examples/api_request_parallel_processor.py），引入时几乎原样保留 ——
# 属第三方代码：改 bug 需对照上游 diff，勿与业务逻辑耦合。
# 在本仓库的唯一调用点: AsyncOpenaiProcessor.process_structured_ouputs_requests
# （src/api_requests.py），负责表格序列化等大批量结构化请求的限流并发调度。
#
# 集成契约（调用方必须遵守）:
#   1. requests_filepath 每行一个 JSON: {API 参数..., "metadata": {...}}；
#      metadata.original_index 由调用方写入，用于消费端重排结果顺序 ——
#      本脚本并发写结果文件，行顺序与请求顺序无关；
#   2. 结果文件每行 [request, response, metadata]（失败行为 [request, 错误列表, metadata]），
#      AsyncOpenaiProcessor 逐行容错解析并按 original_index 还原顺序；
#   3. 速率预算按「每分钟请求数 + 每分钟 token 数」两个独立窗口维护
#      （随时间线性回填的令牌桶），参数应取账号限额的 50%~75% 留出余量。
# ---- 集成说明结束 ----
"""
